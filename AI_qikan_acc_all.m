function results = AI_qikan_acc_all()
% 跑五种模拟视野的结果
%AI_QIKAN_ACC_ALL Run the five visual-field simulation scripts together.
%
...
function settings = AI_default_settings()
    ...
    settings.excelOutputFile = fullfile(settings.excelOutputDir, 'AI_qikan_acc_indices.xlsx');
    settings.matOutputFile = fullfile(settings.scriptDir, 'AI_qikan_acc_all_results.mat');
...
function scenarioResult = AI_run_one_scenario(cfg, settings)
    ...
    accAccum = nan(nTrain, nTest, nFold, subCount);
    probMeanAll = cell(nTrain, nTest, nFold, subCount);
    probAccumAll = cell(nTrain, nTest, nFold, subCount);
        ...
        for train_i = 1:nTrain
            ...
            for test_i = 1:nTest
                ...
                for fold_i = 1:nFold
                    ...
                    [probMean, probAccum, labelLegacy, labelAccum, vfIndex] = ...
                    ...
                    accAccum(train_i, test_i, fold_i, sub_i) = ...
                        mean(labelAccum == cfg.expectedLabel);

                    probMeanAll{train_i, test_i, fold_i, sub_i} = probMean;
                    probAccumAll{train_i, test_i, fold_i, sub_i} = probAccum;
                    vfIndexAll{train_i, test_i, fold_i, sub_i} = vfIndex;

                    fprintf('  train=%d, test=%d, fold=%d, acc_mean=%.3f, acc_accum=%.3f\n', ...
...
function AI_save_outputs(results, settings, scenarios)
    ...
    for scenario_i = 1:numel(scenarios)
        ...
        probAccumAll.(scenarioName) = results.(scenarioName).probAccumAll;
        vfIndexAll.(scenarioName) = results.(scenarioName).vfIndexAll;
    end

    save(settings.matOutputFile, 'probMeanAll', 'probAccumAll', ...
...
function summaryTable = AI_build_summary_table(results, settings, scenarios)
            ...
            for test_i = 1:nTest
                for fold_i = 1:nFold
                    for sub_i = 1:nSub
                        ...
                        accAccum(row_i) = AI_round4(oneResult.accAccum(train_i, test_i, fold_i, sub_i));
    ...
    summaryTable = table(subjectIndex, subjectCode, subjectPath, ...
        ...
        testTrials, foldNum, accLegacy, accAccum, VFI, MD_loss, PSD, ...
        LEI, REI);
end
...
function [probMean, probAccum, labelLegacy, labelAccum, vfIndex] = ...
    ...
    for point_i = 1:pointNum
        ...
        [probAccum(:, point_i), ~] = ...
            AI_accumulate_prob_dv_geomean(probDv, settings.effectiveN, []);

        probDvAll(:, :, point_i) = probDv;
    end

    [~, labelLegacy] = max(probMean, [], 1);
    [~, labelAccum] = max(probAccum, [], 1);
    vfIndex = AI_dcpm_vf_indices_from_prob(probDvAll, settings.effectiveN);
end
...
function [prob, logScore] = AI_accumulate_prob_dv_geomean(probDv, effectiveN, weights)
    if nargin < 2 || isempty(effectiveN)
        effectiveN = 1;
    end
    if nargin < 3 || isempty(weights)
        weights = ones(1, size(probDv, 2));
    end

    epsVal = 1e-12;
    probDv = max(probDv, epsVal);
    probDv = probDv ./ sum(probDv, 1);

    weights = reshape(weights, 1, []);
    weights = weights ./ sum(weights);

    logScore = sum(log(probDv) .* weights, 2);
    logScore = logScore .* effectiveN;

    logScoreShift = logScore - max(logScore);
    prob = exp(logScoreShift);
    prob = prob ./ sum(prob);
end

function result = AI_dcpm_vf_indices_from_prob(probDvAll, effectiveN)
    [~, ~, pointNum] = size(probDvAll);
    pointProb = zeros(3, pointNum);
    pointLogScore = zeros(3, pointNum);

    for point_i = 1:pointNum
        [pointProb(:, point_i), pointLogScore(:, point_i)] = ...
            AI_accumulate_prob_dv_geomean(probDvAll(:, :, point_i), effectiveN, []);
    end
    ...
    result.pointLogScore = pointLogScore;
