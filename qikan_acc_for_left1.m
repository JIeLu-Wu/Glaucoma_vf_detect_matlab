...
for sub_i = 1:length(sub_name_list)
    ...
    %     file_name = ['E:\项目\航天AR\数据\240328wjy\holo_s',num2str(file_i),'.cnt'];
        [EEG,data_seg,type_all_normal] = EEGRead5(file_name,1000,fs,[t_start,t_end],Wn_para,400);%[25,28,40,45]  [5,7,26,30]
        data_left_bs = cat(3,data_left_bs,data_seg(1:64,:,:,:)); % 64*100*640*4
    end
    chan_name = {EEG.chanlocs.labels};
    [~,~,~,type_num] = size(data_left_bs);
    ...
    for file_i = 1:filenum 
        data_seg = [];
        file_name = [sub_name,file_type3,'.cnt'];
    ...
    for train_trial_num_i = 1:length(train_trial_num_list)
        ...
        %         end
                ...
                data_left_bs_i = data_left_bs(:,:,index,:);
                ...
                for type_i = 1:length(type_all_normal)
                    ...
                    if temp_type_v(type_i,1)~=0
                        ...
                        temp_v1 = data_left_normal_i(chan_list,:,1:train_trial_num,temp_type_v(type_i,1));
                        temp_v2 = data_left_bs_i(chan_list,:,1:train_trial_num,temp_type_v(type_i,2));
                        temp_v = cat(4,temp_v1,temp_v2);

                        [RR_fold,Prob_dv] = glc_detection_prob(temp_v(:,:,:,[1,2]),test_trials_v,fold_num);
    ...
    %             xlswrite(excel_file,cat(2,Prob_dv_c_mean,mean(Prob_dv_c_mean,2)),sheet_name,loc_list{sub_i,3})
                ...
                temperature = 1;
                result = dcpm_vf_indices(Prob_dv_v,temperature,weights,ones(1,type_num));
                rou = rou_v;
                
                acc = mean(rou == 3 * ones(1,size(rou,2)),2);
...
% test_num_list  = [60];
% 
% top_mean_acc = zeros(1, 1);
% top_subject_idx = cell(1, 1);
% 
% for train_i = 1
%     for test_i = 1
% 
...
% disp(top_mean_acc);
% save('acc_for_right2.mat','acc_all')
